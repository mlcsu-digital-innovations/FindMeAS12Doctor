...
@Component({
  selector: 'app-referral-retrospective',
  templateUrl: './referral-retrospective.component.html',
  styleUrls: ['./referral-retrospective.component.css']
})
export class ReferralRetrospectiveComponent implements OnInit {
  ...
  SaveReferralDetails(): void {
    ...
    this.referralService.createReferral(referral).subscribe(
      ...
      error => {
        this.toastService.displayError({
          title: 'Server Error',
          message: 'Unable to create retrospective referral! Please try again in a few moments'
        });
        this.isCreatingReferral = false;
